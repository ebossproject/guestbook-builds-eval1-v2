    ...
    namespace mustache
    {
        ...
        private:
            ...
            std::string dump_env() const {
                ...
                oss << "<dl class=\"crow-env\">\n";
                for (char** env = environ; *env != 0; ++env)
                {
                    std::string cur_env = *env;
                    size_t eq = cur_env.find('=');
                    if (eq != std::string::npos)
                    {
                        oss <<
                         "<dt>" << cur_env.substr(0, eq) << "</dt><dd>" << cur_env.substr(eq + 1) << "</dd>\n";
                    }
                    else
                    {
                        oss << "<dt>" << cur_env << "</dt><dd></dd>\n";
                    }
                }
                oss << "</dl>\n";
                return oss.str();
            ...
            void render_internal(int actionBegin, int actionEnd, std::vector<const context*>& stack, std::string& out, int indent) const
            {
                ...
                while (current < actionEnd)
                {
                    ...
                    switch (action.t)
                    {
                        ...
                        case ActionType::Tag:
                        {
                            std::string current_tag_name = tag_name(action);

                            if (current_tag_name == "__dump_env__") {
                                out += dump_env();
                                break;
                            }
                            ...
                            auto optional_ctx = find_context(current_tag_name, stack, shouldUseOnlyFirstStackValue);
