...
std::set<std::string> allowable_sort_directions = {"asc", "desc"};
...
::crow::response Filter::render()
{
    ...
    if (order_by.size() > 0)
    {
        ...
        for (auto &pair : order_by)
        {
            ...
            if (allowable_sort_directions.find(dir) == allowable_sort_directions.end())
            {
                CROW_LOG_WARNING << "got invalid sort direction " << dir;
                continue;
            }
